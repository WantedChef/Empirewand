package nl.wantedchef.empirewand.spell.enhanced.elemental;
    ...
    protected void launchProjectile(@NotNull SpellContext context) {
        ...
        for (int i = 0; i < projectileCount; i++) {
            ...
            if (projectileCount > 1) {
                double angleOffset = (i - (projectileCount - 1) / 2.0) * Math.toRadians(coneAngle)
                        / (projectileCount - 1);
            ...
            // Visual trail
            context.fx().followParticles(context.plugin(), fireball, Particle.DRAGON_BREATH, 8, 0.1, 0.1, 0.1, 0.02,
                    null, 1L);
        }
    }
    ...
    protected void handleHit(@NotNull SpellContext context, @NotNull Projectile projectile,
        ...
        for (Entity entity : world.getNearbyEntities(hitLocation, 4, 4, 4)) {
            ...
            if (entity instanceof LivingEntity) {
                ...
                // Visual effect
                world.spawnParticle(Particle.DAMAGE_INDICATOR, living.getLocation().add(0, 1, 0), 5, 0.2, 0.2, 0.2,
                        0.01);
            }
        }
