...
/**
 * RagfCloud - Cloud-AoE like the known "cloud/pulse" approach from Empire combos:
 * stationary area with pulsing DOT and light pull/knock, in aggressive Ragftagar style.
 */
public final class RagfCloud extends Spell<Void> implements ToggleableSpell {
    ...
    public static class Builder extends Spell.Builder<Void> {
        public Builder(EmpireWandAPI api) {
            super(api);
            name = "Ragf Cloud";
            description = "Cloud-AoE with pulsing DOT and light pull/knock, in aggressive Ragftagar style. Stationary area effect.";
        ...
        public @NotNull Spell<Void> build() {
            return new RagfCloud(this);
        }
    }

    private RagfCloud(Builder builder) {
        super(builder);
    }
    ...
    private final class CloudData {
        ...
        CloudData(Player player, SpellContext context) {
            ...
            player.sendMessage(Component.text("§c⚡ §7Ragf Cloud deployed. Brute pressure zone created."));
        ...
        void stop() {
            ticker.cancel();
            pulseTicker.cancel();
            player.sendMessage(Component.text("§c⚡ §7Ragf Cloud dissipated."));
        }
        ...
        private void spawnCloudParticles() {
            ...
            for (int i = 0; i < 20; i++) {
                ...
                // Alternate between different particle types for aggressive effect
                if (tickCounter % 12 < 4) {
                    world.spawnParticle(Particle.FLAME, particleLoc, 1, 0.1, 0.1, 0.1, 0.01);
                } else if (tickCounter % 12 < 8) {
                    world.spawnParticle(Particle.LAVA, particleLoc, 1, 0.1, 0.1, 0.1, 0.01);
                } else {
                    world.spawnParticle(Particle.SMOKE, particleLoc, 1, 0.1, 0.1, 0.1, 0.02);
                }
            }
            
            // Create occasional explosion particles for aggressive effect
            if (Math.random() < 0.3) {
                world.spawnParticle(Particle.EXPLOSION, cloudLocation, 3, 1.0, 0.1, 1.0, 0.1);
            }
            
            // Create large smoke particles
            if (Math.random() < 0.4) {
                world.spawnParticle(Particle.CAMPFIRE_COSY_SMOKE, cloudLocation, 2, 0.8, 0.1, 0.8, 0.02);
            }
        }
        
        private void pulse() {
            World world = cloudLocation.getWorld();
            double pulseDamage = spellConfig.getDouble("values.pulse-damage", 2.5);
            ...
            for (Entity entity : world.getNearbyEntities(cloudLocation, cloudRadius, cloudRadius, cloudRadius)) {
                if (entity instanceof LivingEntity && entity != player && !entity.isDead()) {
                    ...
                    // Apply knockback effect if entity is near center
                        ...
                        livingEntity.setVelocity(knockbackDirection.multiply(knockbackStrength).setY(0.2));
                    }
                    ...
                    world.spawnParticle(Particle.DAMAGE_INDICATOR, entityLoc.add(0, 1, 0), 3, 0.2, 0.2, 0.2, 0.1);
                    world.spawnParticle(Particle.FLAME, entityLoc, 5, 0.3, 0.3, 0.3, 0.03);
                }
            }
            ...
            // Play pulse sound
            world.playSound(cloudLocation, Sound.ENTITY_GENERIC_EXPLODE, 0.7f, 1.5f);
        }
    }
}