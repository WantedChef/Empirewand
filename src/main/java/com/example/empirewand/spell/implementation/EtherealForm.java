...
public class EtherealForm implements Spell {
    ...
    public void execute(SpellContext context) {
        ...
        int duration = Math.max(1, context.config().getSpellsConfig().getInt("ethereal-form.values.duration-ticks", 100)); // default 5s
        ...
        player.getPersistentDataContainer().set(Keys.ETHEREAL_ACTIVE, PersistentDataType.BYTE, (byte) 1);
        long nowTicks = player.getWorld().getFullTime();
        player.getPersistentDataContainer().set(Keys.ETHEREAL_EXPIRES_TICK, PersistentDataType.LONG, nowTicks + duration);
